...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::table('barangays', function (Blueprint $table) {
            $table->string('barangay_chairman')->nullable()->after('district');
            $table->string('sk_chairman')->nullable()->after('barangay_chairman');
        });
                    ...
                    foreach ($brgys as $item) {
                        ...
                        if ($normDb === $normJson) {
                            DB::table('barangays')
                                ->where('id', $b->id)
                                ->update([
                                    'barangay_chairman' => $item['chairman'],
                                    'sk_chairman' => $item['sk']
                                ]);
                            break 2;
                        }
    ...
    {
        Schema::table('barangays', function (Blueprint $table) {
            $table->dropColumn(['barangay_chairman', 'sk_chairman']);
        });
    }
};
